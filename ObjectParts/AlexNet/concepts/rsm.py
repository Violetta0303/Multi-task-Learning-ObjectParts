...
model = AlexNet(num_concepts=473, init_weights=False).to(device)
model.load_state_dict(torch.load('./AlexNet_Concepts.pth'))
model.eval()
...
with torch.no_grad():
    for images, concept_labels in loader:
        images = images.to(device)
        concepts, activations = model(images)
        ...
        for layer_index, layer_activation in enumerate(activations):
            ...
            generate_rsm_heatmap(rsm, labels, f"Layer_{layer_index + 1}_Concepts")
